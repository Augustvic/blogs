...
服务引入的最终目的是创建代理类。在创建代理类之前需要构建好消费端的 Invoker。
...
开启 NettyClient 客户端的步骤请参考 [Dubbo 协议与通信过程](https://github.com/Augustvic/Blogs/tree/master/Dubbo/md/dubbo/12.md)。
...
下面代码的第 75 行之前主要完成了获取 key 和检查缓存的工作。从第 75 行的注释开始进入创造代理类和 Proxy 类的核心流程。创造代理类的流程和 [服务暴露](https://github.com/Augustvic/Blogs/tree/master/Dubbo/md/dubbo/3.md) 中用 Wrapper.getWrapper 创造包装类、[SPI](https://github.com/Augustvic/Blogs/tree/master/Dubbo/md/dubbo/2.md) 中创造自适应扩展点的流程差不多。
