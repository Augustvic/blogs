...
HashMap 本质上是一个数组，数组用来存储 Node 节点，Node 节点中保存 key 和 value。key 在所有的 key 中是唯一的，value 则可以重复。根据 key 的哈希值决定把 Node 放在哪个位置。

但是 HashMap 不完全是数组，因为数组的一个槽只能代表一个元素，但 HashMap 的一个槽有时候需要存储很多个元素（在产生哈希冲突的时候）。

对于哈希冲突的场景， JDK8 的 HashMap 有两种存储方式，链表和红黑树。需要注意的是，当数组容量大于 64 且某个槽中的节点数达到 8 时，当前槽中的链表将转变为红黑树，而且同时维护（双向）链表结构，只是多了个红黑树结构。如果数组容量小于 64 时发生哈希冲突，直接扩容。

多线程环境下不可以直接使用 HashMap，最好用 ConcurrentHashMap 代替。 
 ...
 **加载因子**大时，优点是填满的元素多，空间利用率高，加载因子小时，优点是冲突的概率小，链表较短，查找效率变高，但可能会进行频繁的扩容操作，也会消耗性能。默认加载因子为 0.75，是时间效率和空间效率的一种平衡。

注意 size 表示映射的数量，而不是桶的数量。size 大于阈值时执行扩容操作。

真正初始化哈希表（初始化存储数组 table）不是在构造函数中，而是在添加第一个键值对的时候。
...
Node 类是 HasMap 最简单的节点类型，用于桶内数据结构为链表时的情景。当桶内的数据结构转化成红黑树时，节点也转化成红黑树的节点类型 TreeNode。TreeNode 节点中包括了红黑树的一系列复杂操作，如变色、旋转等。
...
hash 函数重新计算对象的 hash 值，用于计算定位元素在数组中的位置，将 hashCode 的高 16 位与 hashCode 进行异或运算，是为了在 table 的 length 较小的时候，让高位也参与运算，并且不会有太大的开销。
...
put 操作将指定的 key 和 value 作为节点添加到 Map 里面。putVal 函数中，首先判断 table 是否已经初始化，是否需要扩容，然后通过 key 的 hash 值找到要将新节点插入的桶。此时有两种情况需要讨论，一种是已经存在 key 对应的节点。此情景下将 value 设置成指定的 value，并返回旧的 value 值。另一种是 Map 中本身不存在指定 key 对应的节点，那么将新的节点插入到链表尾部或者红黑树适当位置即可。

计算键值对在桶内的位置时，使用 hash & (n - 1) 计算，即只取 hash 值的低位掩码（为了匹配 table 数组大小），所以之前说过，在计算 hash 值的时候纳入了高 16 位，是为了让高 16 位也加入运算，从而避免 hash 值冲突。这个计算方式其实和 hash % n 的结果一样，之所以选用位运算是为了提高效率。
...
### 为什么 HashMap 桶内不直接使用红黑树，首先使用双向链表

这是内存占用与存储桶内查找复杂性之间权衡的结果。大多数哈希函数将产生非常少的冲突，因此为节点数较少的桶维护树结构将是非常昂贵的。

### 红黑树与 AVL 树

* AVL严格要求左右两个子树的高度差的绝对值不超过 1，结构上比红黑树更平衡，时间效能针对读取而言更高。红黑树在查询性能上不如 AVL 树。

* 红黑树是用非严格的平衡来换取增删节点时候旋转次数的降低，任何不平衡都会在三次旋转之内解决。AVL 树是严格平衡树，在增加或者删除节点的时候，会进行大量的平衡度计算，根据不同情况，旋转的次数比红黑树要多。

* 实际应用中，若搜索的次数远远大于插入和删除，那么选择 AVL 树，如果搜索，插入删除次数几乎差不多，应该选择红黑树。

### JDK 1.8 相对于 JDK 1.7

**数据结构**

为了提高 HashMap 的性能，引入了红黑树。在桶内查找，如果链表过长时间代价为 O(n)，红黑树时间代价为 O(logn)。当链表长度大于 8 时，将双向链表转变为红黑树。（查找的时候，首先判断桶内结构是否为红黑树，如果是红黑树则按照红黑树的方式查找，如果不是红黑树，才按照链表的方式查找。这样做是因为如果桶内结构如果是红黑树，它同时也会是双向链表，若首先按照链表方式查找，则永远用不到红黑树结构。）

**计算哈希值**：

调用 hash(key) 函数计算哈希值，JDK 1.7 中使用了 4 次位运算和 5 次异或运算，JDK 1.8 中只做了 1 次位运算和 1 次异或运算。所有处理的根本目的，都是为了提高存储键值对的数组下标位置的随机性和分布均匀性，尽量避免出现哈希冲突和 hash 值冲突。

**扩容**：

* 插入一个元素时如果会引发扩容，JDK 1.7 是先扩容再插入，JDK 1.8 是先插入再扩容。

* JDK 1.7 中对每一个 key 采用原来的计算方法（hashCode & (length - 1）进行计算，JDK 1.8 中根据新增参与运算位进行计算（原位置 / 原位置 + 扩容前数组容量），先把链表分成“留在原位置”和“转移到新位置”两个部分，然后再转移。

* JDK 1.7 中使用头插法，将原位置数据移到后一位，再插入数据到该位置，会出现逆序，环形链表死循环等问题；JDK 1.8 中直接插入链表尾部 / 红黑树，不会出现逆序，环形链表死循环等问题。

### hashCode() 和 equals()

key 如果为 Object 类型，则需要实现 hashCode 方法和 equals 方法。

hashCode 方法会用来计算需存储数据的存储位置，如果实现不恰当会导致严重的哈希碰撞问题。
equals 方法用来比较两个 key 是否相等，保证 key 在哈希表中的唯一性。

hash 值相等的时候，key 不一定相等，只能通过 equals 来判断 key 是否相等。

### 为什么容量总是 2 的幂

**用 & 代替 %，提升性能**

计算 key 落在数组中哪一个桶里时，用 key 的 hash 值和 table 的长度 length 作取余运算，而仅当 length 为 2 的幂时，才有 key.hash % table.length 等于 key.hash & (table.length - 1)，原因如下：任何 2 的幂除最高位是 1 之外，其它位都为 0，减 1 之后，最高位变成 0，比最高位低的所有位次变成 1，此时和 key.hash 作 & 运算，相当于保留 hash 的低位，即相当于对 length 取余。由于位运算的效率要远远高于 % 运算，所以选择了位运算而不是 % 运算。

**分布更均匀**

如果 table.length 为 2 的幂，执行 key.hash & (table.length - 1) 时得到的值为 key.hash 的后面几位。所以只需要保证 hash 值足够分散即可。

极端情况下若 table.length - 1 的后面几位全部为 0，哈希冲突将会非常严重。

### 哈希冲突

**避免哈希冲突**

哈希算法：一次位运算加一次异或（高低 16 位参与运算）

```java
(key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16)
```

扩容机制：当哈希表的大小大于等于扩容阈值（容量乘负载因子）时，对哈希表扩容。负载因子越小，发生碰撞的概率越低。
...
在 JDK 1.7 中，对 HashMap 扩容时使用了头插法，多线程同时操作可能形成环形链表。

在 JDK 1.8 中使用尾插法，不会出现链表逆序、倒置的情况，故不会出现环形链表的情况，但是在 put 操作和 get 操作中没有加同步锁，无法保证上一时间片获取到的值，此时没有改变。例如，两个线程同时进行插入操作，一个线程 A 获取插入位置（链表尾部）之后等待时间片，另一个线程 B 完成插入操作，等到 A 进行插入操作时，其保存的位置已经不是链表尾部，此时插入将会覆盖线程 B 的数据。

可以通过 Collections 类的静态方法 synchronizedMap 获得线程安全的 HashMap（效率低），也可以使用 HashTable（效率低） 或 ConcurrentHashMap。
