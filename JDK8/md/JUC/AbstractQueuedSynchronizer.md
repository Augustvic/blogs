...
Condition 类仍然使用 Node 节点作为基础数据结构，每一个 Condition 队列的构建主要包含两个属性，分别时作为队列头结点的 head 和作为队列尾节点的 tail：
...
在 addConditionWaiter 方法中，把当前线程添加到 Condition 队列，且没有直接使用原来的 Node 节点，而是创建新的节点容纳线程。也就是说这时候同步队列和 Condition 队列中分别有一个节点保存了该线程。
...
唤醒 Condition 中的节点实际上是把节点加入到同步 (Sync) 队列中，同时设置节点的状态。这些都在 signal 相关函数中实现。当节点（线程）被从 Condition 中唤醒后，可以通过 predecessor() 方法获取到前驱节点。
...
awaitUninterruptibly 方法和 await 方法相比，区别是不即时响应中断。如果被中断了，在进入同步队列之后，再自行中断。
...
awaitNanos 方法在 await 方法的基础上，加上等待的时间限制。同样采用自旋的方式检查是否已经在同步队列中，如果指定等待时间片被耗尽，则退出自旋，开始竞争同步状态。
